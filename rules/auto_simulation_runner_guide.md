...
| `compile_video` | `true` | 是否在每视角下用 ffmpeg 合成 mp4（需安装 ffmpeg）。 |
...
| `num_render_timesteps` | `0` | `0` 或 ≥ 仿真总帧：每仿真步都输出；否则在时间上均匀取 K 个时刻；RGB / 视角应力 / 2D 轨迹与此对齐。 |
| `white_bg` | `false` | 白底渲染。 |
...
| `output_view_tracks2d` | `false` | 每视角 2D 投影轨迹 `tracks_2d/*.npz`。 |
