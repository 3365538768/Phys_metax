...
from my_utils.arch4_lmdb import read_arch4_lmdb_view_tensors, unpack_uint8_thwc
...
def _read_lmdb_arrays_full_frames(
    ...
    *,
    ...
    返回 dict，键为 rgb/stress/flow/force_mask，值形状 [V,3,T,H,W]。
    """
    try:
        import lmdb
    except ImportError as e:
    ...
    env_path = sample / lmdb_env_subdir
    meta = _read_meta(env_path)
    views = list(meta.get("views") or [])[: int(max_views)]
    if not views:
        raise ValueError(f"__meta__ 里无可用 views: {env_path}")

    buf: Dict[str, List[np.ndarray]] = {m: [] for m in (_REQ_MODALITIES + _OPT_MODALITIES)}
    env = lmdb.open(str(env_path), readonly=True, lock=False, readahead=True, max_readers=64)
    try:
        with env.begin() as txn:
            for v in views:
                # 先读四个必需模态
                for mod in _REQ_MODALITIES:
                    blob, key_used = _get_lmdb_blob_exact(
                        txn, view_name=v, mod=mod
                    )
                    if blob is None:
                        raise KeyError(f"缺少键: {key_used!r}")
                    tchw_u8 = unpack_uint8_thwc(bytes(blob))  # [T,3,H,W]
                    chw_t = np.transpose(tchw_u8.astype(np.float32) / 255.0, (1, 0, 2, 3))
                    buf[mod].append(np.ascontiguousarray(chw_t, dtype=np.float32))

                # 再读 object_mask（可选）
                blob_obj, _obj_key = _get_lmdb_blob_exact(
                    txn, view_name=v, mod="object_mask"
                )
                if blob_obj is None:
                    # 兼容旧数据：object_mask 缺失时，用全 1 掩码（不抑制任何区域）
                    shape_ref = buf["force_mask"][-1].shape
                    obj = np.ones(shape_ref, dtype=np.float32)
                else:
                    tchw_u8 = unpack_uint8_thwc(bytes(blob_obj))  # [T,3,H,W]
                    obj = np.transpose(tchw_u8.astype(np.float32) / 255.0, (1, 0, 2, 3))
                    obj = np.ascontiguousarray(obj, dtype=np.float32)
                buf["object_mask"].append(obj)
    finally:
        env.close()

    # 强约束：同一模态下所有视角应有一致形状，便于堆叠成 [V,3,T,H,W]
...
def read_lmdb_arrays(
    ...
    *,
    ...
    - num_frames>0：重采样到指定帧数（兼容旧接口）
    """
    if num_frames is None or int(num_frames) <= 0:
        return _read_lmdb_arrays_full_frames(
    ...
    if img_size is None or int(img_size) <= 0:
        meta = _read_meta(Path(sample_dir) / lmdb_env_subdir)
        img_size = int(meta.get("img_size") or 0)
    if int(img_size) <= 0:
        raise ValueError("img_size 无效；请传入 --img_size 或确保 __meta__.img_size 存在")

    env_path = Path(sample_dir) / lmdb_env_subdir
    rgb_l, s_l, f_l, fm_l = read_arch4_lmdb_view_tensors(
        env_path=env_path,
        num_frames=int(num_frames),
        img_size=int(img_size),
        max_views=int(max_views),
    )
    # object_mask 走独立读取（保持 force_mask 与 object_mask 语义解耦）
    full_arrays = _read_lmdb_arrays_full_frames(
        sample_dir=sample_dir,
        max_views=int(max_views),
        lmdb_env_subdir=lmdb_env_subdir,
    )
    obj_full = full_arrays["object_mask"]  # [V,3,T,H,W]
    t_target = int(num_frames)
    obj_resampled = []
    for vi in range(int(obj_full.shape[0])):
        x = obj_full[vi]  # [3,T,H,W]
        t0 = int(x.shape[1])
        if t0 == t_target:
            obj_resampled.append(np.ascontiguousarray(x, dtype=np.float32))
        else:
            idx = np.linspace(0, t0 - 1, t_target).astype(np.int64)
            obj_resampled.append(np.ascontiguousarray(x[:, idx, :, :], dtype=np.float32))
    return {
        "rgb": np.stack(rgb_l, axis=0),
        "stress": np.stack(s_l, axis=0),
        "flow": np.stack(f_l, axis=0),
        "force_mask": np.stack(fm_l, axis=0),
        "object_mask": np.stack(obj_resampled, axis=0),
    }
...
class LmdbGtDataset(Dataset):
        ...
        *,
        ...
        for d in list_lmdb_samples(self.split_root, self.lmdb_env_subdir):
            if (d / "gt.json").is_file():
                ...
                self._action_name_by_sample[d.name] = str(gt.get("action", "unknown")).strip() or "unknown"
        if not self.samples:
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        out = {
            "sample_id": d.name,
            "rgb": torch.from_numpy(np.ascontiguousarray(arrays["rgb"])),
            "stress": torch.from_numpy(np.ascontiguousarray(arrays["stress"])),
            "flow": torch.from_numpy(np.ascontiguousarray(arrays["flow"])),
            "force_mask": torch.from_numpy(np.ascontiguousarray(arrays["force_mask"])),
            "object_mask": torch.from_numpy(np.ascontiguousarray(arrays["object_mask"])),
            "params": params,
            "params_dict": gt,
